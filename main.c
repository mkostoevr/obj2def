...
static uint32_t get32(const char *buf, size_t offset) {
...
static EpepCoffRelocation get_relocation_for_section_and_offset(Epep *epep, EpepSectionHeader *sh, size_t offset) {
...
static char *read_string(Epep *epep, size_t section_index, size_t offset) {
...
static int gendef(const char *obj_path, const char *outname) {
	...
	for (size_t sym_i = 0; sym_i < epep.coffFileHeader.NumberOfSymbols; sym_i++) {
		...
		if (!strcmp(name, "_EXPORTS") || !strcmp(name, "EXPORTS")) {
			...
			// The export table is a sequence of { char *name; void *symbol; } (32 bit)
			// Compiler writes here offsets relateive to some symbol instead of
			// the pointers we need. So in order to get poitner to the import name
			// we should find relacation for the pointer, find relative to what symbol
			// the relocation is and get the pointer by adding the given offset to the
			// symbol's definition offset.
...
static int arg_got_flag(int argc, char **argv, ...) {
	char *arg_names[8];
	int arg_name_c = 0;
...
static int usage(char *name, char *remark) {
	if (remark) {
		printf("Error: %s\n\n", remark);
	}
